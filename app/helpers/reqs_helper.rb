module ReqsHelper
  ...
  def req_email_content(new_req)
    ...
    email_string += " You can always view the latest Req of the Week by visiting http://" + request.host + url_for(req_of_the_week_path) + "." 
  ...
  def req_generate_mail
    return mail_to "test@test.com", "Company email", subject: "Req of the Week - " + Req.last.title + " - " + Date.today.to_formatted_s(:rfc822), body: req_email_content(Req.last), id: "autoEmail"
  end
  ...
  def generate_friend_mail
    return mail_to "", "<span class='fa fa-envelope-o fa-lg'></span>".html_safe, subject: "Job opening at GuideSpark: " + Req.last.title, body: friend_email_content(Req.last), class: 'btn btn-sm btn-primary'
  end
